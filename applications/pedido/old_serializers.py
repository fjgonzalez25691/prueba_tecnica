...
class DetallePedidoUpdateSerializer(serializers.ModelSerializer):
...
class PedidoUpdateSerializer(serializers.ModelSerializer):
    ...
    def get_detalles(self,obj):
        consulta = DetallePedido.objects.linea_pedido_update(obj.id)
        articulos_serializados = DetallePedidoUpdateSerializer(consulta, many=True).data
        return articulos_serializados
    
class ProcesoModificarPedidoSerializer(serializers.Serializer):
    """Serializador para el proceso de venta"""
